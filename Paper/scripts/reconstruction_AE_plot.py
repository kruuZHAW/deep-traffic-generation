...
with plt.style.context("traffic"):
    fig, ax = plt.subplots(
        1, 2, figsize=(13, 7), subplot_kw=dict(projection=EuroPP())
    )

    ax[0].set_title("FCAE reconstruction", pad=20, fontsize=20)
    reconstruction_fcae[0].plot(ax[0], lw=2)
    reconstruction_fcae[1].plot(ax[0], lw=2)

    ax[1].set_title("TCAE reconstruction", pad=20, fontsize=20)
