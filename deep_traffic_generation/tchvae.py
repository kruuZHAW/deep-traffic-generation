...
from deep_traffic_generation.core import TCN, FCN, HVAE, cli_main
...
longitude --info_index -1
    ...
    _required_hparams = HVAE._required_hparams + [
        "sampling_factor",
        "kernel_size",
        "dilation_base",
        "n_components",
    ...
    def __init__(
        ...
        # Those next 3 encode x and z2 to z1
        ...
        self.encoder_z1_z2 = FCN(
            input_dim=self.hparams.encoding_dim,
            out_dim=self.hparams.encoding_dim,
            h_dims=[self.hparams.encoding_dim],
            h_activ=nn.ReLU(),
            dropout=0.2,
        )

        self.encoder_z1_joint = FCN(
            input_dim=self.hparams.encoding_dim * 2,
            out_dim=self.hparams.encoding_dim,
            h_dims=[],
            h_activ=nn.ReLU(),  # ReLU dans version 2, sigmoid dans 0 et 1
            dropout=0,
        )
...
if __name__ == "__main__":
    cli_main(TCHVAE, TrafficDataset, "image", seed=42)
